...
| FR-MGT-010 | Database migrations MUST be validated against a real PostgreSQL in CI. | PRs touching `migrations/` or `docker/` run the full up → assertions → down → up cycle; Chinese segmentation asserted when the zhparser migration is present. |
