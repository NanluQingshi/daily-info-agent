...
func buildTestScheduler(
	...
	mgr := fetcher.NewManager([]fetcher.Fetcher{rssMock}, nil, nil, cacheFile, slog.Default())
...
func TestScheduler_RunDefault_AllSourcesFail_FatalErrorSet(t *testing.T) {
	...
	cacheFile := filepath.Join(t.TempDir(), "dedup.json")
	mgr := fetcher.NewManager([]fetcher.Fetcher{errFetcher}, nil, nil, cacheFile, slog.Default())
